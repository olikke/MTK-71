...
type
  ...
  private
    ...
    procedure FullState;
  public
    property    VideoExh:boolean  read fVideo write fVideo;
...
begin
  ...
  if St<>Red then
    if (not fExh1) and (not fExh2) then St:=Red else
    if (not fExh1) or  (not fExh2) then St:=Yellow;
...
begin
  ...
  if TotalSize<>0 then
    ...
    fHDDUse[num]     :=0;
  if fHDDUse[num]<=GreenLimit  then fHDDColor[num]:=_Green else
  if fHDDUse[num]<=YellowLimit then fHDDColor[num]:=_Yell else
                                    fHDDColor[num]:=_Red;
end;
...
begin
  ...
  begin
    ...
    fExh3:=fPing.Exh3;
    fExh4:=fPing.Exh4;
    if first<3 then INC(first);
    if first=2 then PostMessage(fMain.Handle,WM_NOTIFY_MAIN,1,1);
    CalcAllHDDParameter;
    FullState;
    if fimage<>0 then
    PostMessage(fimage,WM_NOTIFY_2322,0,0);
