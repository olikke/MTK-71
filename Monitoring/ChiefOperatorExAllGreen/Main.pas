...
var i,j:integer;
    ...
    Button:TMyPanel;
begin
  //надо сделать так, чтоб перерисовывалось только когда надо, а не каждый раз
  ...
  case PageControl.ActivePage.PageIndex of
    ...
    2:  begin
           for i:=1 to 8 do
            if bdKU[i].Use then
            begin
              SmartDeviceStateByName('vk'+IntToStr(i),dsGreen);
              SmartDeviceStateByName('vk'+IntToStr(i)+'_v', dsGreen);
              SmartDeviceStateByName('vk'+IntToStr(i)+'_v0',dsGreen);
            end;
          SmartDeviceState(vt3401,dsGreen);
          SmartDeviceState(vt34011,dsGreen);
          SmartDeviceState(vt34012,dsGreen);
          SmartDeviceState(vt2322,dsGreen);
          if TM2322.VideoExh then SmartDeviceState(vt2322ve,dsGreen)
                             else SmartDeviceState(vt2322ve,dsGreen);
          if TM2408.DataExh then
          begin
            SmartDeviceState(vt2408,dsGreen);
            if TM2408.Video then SmartDeviceState(vt2408ve,dsGreen)
                            else SmartDeviceState(vt2408ve,dsGreen);
          end else
          begin
            SmartDeviceState(vt2408,dsGreen);
            SmartDeviceState(vt2408ve,dsGreen);
          end;
          SmartDeviceState(vt73021,dsGreen);
          SmartDeviceState(vt73022,dsGreen);
        //  RepaintTable;
          for i:=0 to 13 do
          begin
            if EM7210[i].Use then
            begin
              ...
              if VE then SmartDeviceStateByName('vpx'+IntToStr(i+1),dsGreen)
                    else SmartDeviceStateByName('vpx'+IntToStr(i+1),dsGreen);
              SmartDeviceStateByName('vp'+IntToStr(i+1),dsGreen);
            end;
          end;     
          for i:=0 to 13 do
          begin
            ...
            begin
              ...
              if VE then SmartDeviceStateByName('vbx'+IntToStr(i+1),dsGreen)
                    else SmartDeviceStateByName('vbx'+IntToStr(i+1),dsGreen);
              SmartDeviceStateByName('vb'+IntToStr(i+1),dsGreen);
            end;
          end;

          for i:=1 to 4 do
          begin
            SmartDeviceStateByName('vt2437'+IntToStr(i),dsGreen);
            for j:=1 to 4 do
            begin
              case j of
                1: SmartDeviceStateByName('vt2437'+IntToStr(i)+IntToStr(j),dsGreen);
                2: SmartDeviceStateByName('vt2437'+IntToStr(i)+IntToStr(j),dsGreen);
                3: SmartDeviceStateByName('vt2437'+IntToStr(i)+IntToStr(j),dsGreen);
                4: SmartDeviceStateByName('vt2437'+IntToStr(i)+IntToStr(j),dsGreen);
              end;
            end;
          end;
        end;
