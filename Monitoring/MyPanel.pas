...
type
  ...
  TState = (dsGreen,dsRed,dsYellow,dsGray,dsWhite,dsBlue);
  ...
  private
    ...
    procedure SetDeviceState(Value:TState);
    procedure SetTextposition(Poss:TTextPosition);
  protected
  ...
  published
    ...
    property DeviceState: TState read FDeviceState write SetDeviceState;
...
procedure Register;
begin
  RegisterComponents('Xi Controls', [TMyPanel]);
end;
...
begin
  ...
  case TextPosition of
    tpLeft: i:= BevelWidth + 2;
    tpRight: i:= ClientWidth - BevelWidth - 2 -ScrBmp.Canvas.TextWidth(Caption);
    tpCenter: i:= (ClientWidth - ScrBmp.Canvas.TextWidth(Caption)) div 2;
  end;
...
procedure TMyPanel.DrawBevel(Canvas: TCanvas; Rect: TRect; LightColor, DarkColor: TColor);
begin
  Canvas.Pen.Width:= 1;
...
procedure TMyPanel.DrawFrame(Canvas: TCanvas; Offset, FrameWidth: Integer; LightColor, DarkColor: TColor);
var
  i: integer;
begin
...
procedure TMyPanel.SetColors(Index: Integer; Value: TColor);
begin
  case Index of
    0: FColorFace:= Value;
    1: FColorGrad:= Value;
...
procedure TMyPanel.SetFillDirection(Value: TFillDirection);
begin
  FFillDirection:= Value;
  Invalidate;
end;

procedure TMyPanel.SetDeviceState(Value: TState);
begin
  FDeviceState:=Value;
  case FDeviceState of
    dsGreen: begin
               ...
               FColorFace:= $0066DD22;
               FColorGrad:= $0066DD22;
               FColorLight:=$00156F37;
             end;
    dsRed:   begin
               FColorDark:= $0000009D;
               FColorFace:= $005577FF;
               FColorGrad:= $005577FF;
               FColorLight:=$0000009D;
             end;
    dsYellow:begin
               FColorDark:= $00006464;
               FColorFace:= $0000DDFF;
               FColorGrad:= $0000DDFF;
               FColorLight:=$00006464;
             end;
    dsGray:  begin
               FColorDark:= $00626262;
               FColorFace:= $00DAE0DE;
    ...
    dsBlue: begin
               ...
               FColorFace:= $00F0964D;
               FColorGrad:= $00F0964D;
               FColorLight:=$00B35900;
             end;
  end;
  Invalidate;
end;

procedure TMyPanel.SetTextposition(Poss: TTextPosition);
begin
  FTextPosition:=Poss;
  Invalidate;
end;

procedure TMyPanel.SetColorScheme(Value: TColorScheme);
begin
  FColorScheme:= Value;
...
procedure TMyPanel.GradientFillRect(Canvas: TCanvas; Rect: TRect;
                StartColor, EndColor: TColor; Direction: TFillDirection);
var
  Steps: Integer;
