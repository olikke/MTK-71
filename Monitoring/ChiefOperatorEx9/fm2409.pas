...
begin
  ...
  for i:=1 to 8 do
    if f2409.FindComponent('d'+IntToStr(i))<>nil then
       TMyPanel(f2409.FindComponent('d'+IntToStr(i))).DeviceState:=fState[i];
  if fDataExh then DE.Brush.Color:=_Green else DE.Brush.Color:=_Red;
  if First then
    for i:=1 to 8 do
      if fUSE[i] then
        if f2409.FindComponent('d'+IntToStr(i))<>nil then
...
procedure Tf2409.Timer1Timer(Sender: TObject);
begin
